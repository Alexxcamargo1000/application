...
export function TaskContainer() {
  const { tasks, updateTask, isLoading, finishedTask, numberTaskCompleted } =
    useStory((store) => {
      return {
        tasks: store.tasks,
    ...
    <>
      ...
      <div className="flex w-full justify-end pr-4 mt-6">
        <span className="text-sm text-zinc-600">
          {numberTaskCompleted}/{tasks.length}
        </span>
      </div>
      ...
      {numberTaskCompleted === tasks.length && tasks.length !== 0 && (
        <div>
          <button
            ...
            disabled={isLoading}
            className="px-4 py-2 leading-loose  text-sm font-semibold flex items-center gap-2 bg-red-600 hover:bg-red-700 transition-colors animate-fade rounded disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Finalizar tarefa
          </button>
        </div>
